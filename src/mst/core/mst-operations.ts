 ...
 * @returns
 */
export function onSnapshot<S>(target: IStateTreeNode<IType<any, S, any>>, callback: (snapshot: S) => void): IDisposer {
  // check all arguments
  assertIsStateTreeNode(target, 1);
  assertIsFunction(callback, 2);
 ...
 * @returns
 */
export function applyPatch(target: IAnyStateTreeNode, patch: IJsonPatch | ReadonlyArray<IJsonPatch>): void {
  // check all arguments
  assertIsStateTreeNode(target, 1);
...
export function recordPatches(
  ...
  filter?: (patch: IJsonPatch, inversePatch: IJsonPatch, actionContext: IActionContext | undefined) => boolean
): IPatchRecorder {
  // check all arguments
 ...
 * @returns
 */
export function getSnapshot<S>(target: IStateTreeNode<IType<any, S, any>>, applyPostProcess = true): S {
  // check all arguments
  assertIsStateTreeNode(target, 1);
 ...
 * @returns
 */
export function getParentOfType<IT extends IAnyComplexType>(target: IAnyStateTreeNode, type: IT): IT['Type'] {
  // check all arguments
  assertIsStateTreeNode(target, 1);
  assertIsType(type, 2);
...
export function tryReference<N extends IAnyStateTreeNode>(
  ...
  try {
    ...
    if (e instanceof InvalidReferenceError) return undefined;
...
export function isValidReference<N extends IAnyStateTreeNode>(
  ...
  try {
    ...
    if (isStateTreeNode(node)) return checkIfAlive ? isAlive(node) : true;

    throw fail('The reference to be checked is not one of node, null or undefined');
  } catch (e) {
    if (e instanceof InvalidReferenceError) return false;
 ...
 * @returns
 */
export function clone<T extends IAnyStateTreeNode>(source: T, keepEnvironment: boolean | any = true): T {
  // check all arguments
  ...
  return node.type.create(
    ...
    keepEnvironment === true ? node.root.environment : keepEnvironment === false ? undefined : keepEnvironment
 ...
 */
export function walk(target: IAnyStateTreeNode, processor: (item: IAnyStateTreeNode) => void): void {
  // check all arguments
  assertIsStateTreeNode(target, 1);
 ...
 * @returns
 */
export function getPropertyMembers(typeOrNode: IAnyModelType | IAnyStateTreeNode): IModelReflectionPropertiesData {
