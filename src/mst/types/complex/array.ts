...
import {
    _getAdministration, action, IArrayDidChange, IArraySplice, IArrayWillChange, IArrayWillSplice, intercept,
...
> {
  ...
  constructor(name: string, private readonly _subType: IT, hookInitializers: Array<IHooksGetter<IMSTArray<IT>>> = []) {
    super(name);
...
function valueAsNode(childType: IAnyType, parent: AnyObjectNode, subpath: string, newValue: any, oldNode?: AnyNode) {
 ...
 */
export function isArrayType<Items extends IAnyType = IAnyType>(type: IAnyType): type is IArrayType<Items> {
