...
export class CustomType<S, T> extends SimpleType<S | T, S, T> {
  ...
  reconcile(current: this['N'], value: S | T, parent: AnyObjectNode, subpath: string): this['N'] {
    ...
    if (!current.isDetaching) {
      const unchanged =
        current.type === this && (isSnapshot ? value === current.snapshot : value === current.storedValue);
    ...
    const valueToStore: T = isSnapshot ? this.options.fromSnapshot(value as S, parent.root.environment) : (value as T);
