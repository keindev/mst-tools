 ...
 */
import { configure, values } from 'mobx';

import { types } from '../../index';
import { getParent, getSnapshot, hasParent, Instance, recordPatches, unprotect } from '../../mst/index';
...
it('store is deserialized correctly', () => {
  const s = createStore();
...
it('store emits correct patch paths', () => {
  configure({
...
it('box operations works correctly', () => {
  const s = createStore();
  ...
  s.boxes.get('a')!.setName("I'm Bob");
  expect(getSnapshot(s)).toEqual({
    boxes: {
      cc: { id: 'cc', name: 'Rotterdam', x: 100, y: 100 },
      aa: { id: 'aa', name: 'Bratislava', x: 650, y: 300 },
      a: { id: 'a', name: "I'm Bob", x: 0, y: 0 },
      b: { id: 'b', name: 'B', x: 100, y: 100 },
    },
    arrows: [
      { id: 'dd', from: 'cc', to: 'aa' },
