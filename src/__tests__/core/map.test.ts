...
import {
    applyPatch, applySnapshot, detach, getSnapshot, IAnyModelType, IJsonPatch, isStateTreeNode, onPatch, onSnapshot,
    SnapshotOut, unprotect,
} from '../../mst/index';
...
it('#184 - types.map().get(key) should not throw if key doesnt exists', () => {
  const { Factory } = createTestFactories();
  const doc = Factory.create({
    hello: {
      to: 'world',
...
it('#192 - put should not throw when identifier is a number', () => {
...
it('#192 - map should not mess up keys when putting twice', () => {
  const Todo = types.model('Todo', { todo_id: types.identifierNumber, title: types.string });
  const TodoStore = types
    .model('TodoStore', {
      todos: types.optional(types.map(Todo), {}),
...
it('#694 - map.put should return new node', () => {
  configure({
    useProxies: 'never',
  });
...
it('should not throw when removing a non existing item from a map', () => {
  expect(() => {
...
it('should get map keys from reversePatch when deleted an item from a nested map', () => {
  const AppModel = types
    .model('', {
...
it('map expects regular identifiers', () => {
...
it('issue #876 - map.put works fine for models with preProcessSnapshot', () => {
...
it('map can resolve late identifiers', () => {
...
it('get should return value when key is a number', () => {
  const Todo = types.model('Todo', {
...
it('#751 restore from snapshot should work', () => {
  configure({
    enforceActions: 'never',
  });
...
it('#1173 - detaching a map should not eliminate its children', () => {
  const M = types.model('', {});
...
it('#1131 - put with optional identifier', () => {
