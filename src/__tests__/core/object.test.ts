...
import {
    applyAction, applyPatch, applySnapshot, cast, destroy, detach, getParent, getSnapshot, IJsonPatch, isAlive,
    ISerializedActionCall, onAction, onPatch, onSnapshot, resolveIdentifier, setLivelinessChecking, SnapshotOut,
    unprotect,
} from '../../mst/index';
...
it('should emit snapshots for children', () => {
  ...
  const snapshotsP: SnapshotOut<typeof folder>[] = [];
  const snapshotsC: SnapshotOut<typeof folder.files[0]>[] = [];
...
it('should apply and accept null value for types.maybe(complexType)', () => {
...
it('should apply and accept null value for types.maybeNull(complexType)', () => {
  const Item = types.model('Item', {
    value: types.string,
  });
  const Model = types.model('Model', {
...
it('should stop listening to patches patches', () => {
  configure({
    enforceActions: 'never',
  });
  ...
  const patches: IJsonPatch[] = [];
  const disposer = onPatch(doc, patch => patches.push(patch));
...
it('should throw if a replaced object is read or written to', () => {
  configure({
    enforceActions: 'never',
  });
...
it('should warn if a replaced object is read or written to', () => {
  configure({
    enforceActions: 'never',
  });
  ...
  const error = 'You are trying to read or write to an object that is no longer part of a state tree';
...
it('should compose factories with computed properties', () => {
...
it('should compose multiple types with computed properties', () => {
  const { ComputedFactory2, ColorFactory } = createTestFactories();
  const ComposedFactory = types.compose(ColorFactory, ComputedFactory2);
  const store = ComposedFactory.create({ props: { width: 100, height: 200 } });
...
it('methods get overridden by compose', () => {
  const A = types
    .model('', {
...
it('compose should add new props', () => {
  const A = types.model('', {
    count: types.optional(types.number, 0),
  });
  const B = A.props({
...
it('models should expose their actions to be used in a composable way', () => {
  const A = types
    .model('', {
      count: types.optional(types.number, 0),
    })
...
it('compose should be overwrite', () => {
  const A = types
    .model('', {
      name: '',
      alias: '',
...
it('complex map / array values are optional by default', () => {
  expect(types.model('', { todo: types.model('', {}) }).is({})).toBe(false);
  expect(() => types.model('', { todo: types.model('', {}) }).create({} as any)).toThrow();
  expect(types.model('', { todo: types.array(types.string) }).is({})).toBe(true); // TBD: or true?
  expect(getSnapshot(types.model('', { todo: types.array(types.string) }).create({}))).toEqual({ todo: [] });
  expect(types.model('', { todo: types.map(types.string) }).is({})).toBe(true);
  expect(getSnapshot(types.model('', { todo: types.map(types.string) }).create({}))).toEqual({ todo: {} });
});

// === VIEW FUNCTIONS ===
it('view functions should be tracked', () => {
  configure({
    enforceActions: 'never',
  });
...
it('view functions should not be allowed to change state', () => {
  const model = types
    .model('', {
      x: 3,
    })
...
it('should consider primitives as proposed defaults', () => {
  const now = new Date();
...
it('should throw if a non-primitive value is provided and no default can be created', () => {
  expect(() => {
    types.model('', {
      complex: {
        a: 1,
...
it('should not be possible to remove a node from a parent if it is required, see', () => {
...
it('should be possible to remove a node from a parent if it is defined as type maybe', () => {
  const A = types.model('A', { x: 3 });
...
it('should be possible to remove a node from a parent if it is defined as type maybeNull', () => {
  const A = types.model('A', { x: 3 });
...
it('should be possible to share states between views and actions using enhance', () => {
...
it('should throw if any other key is returned from extend', () => {
  // eslint-disable-next-line @typescript-eslint/no-empty-function
...
it('961 - model creating should not change snapshot', () => {
...
it('#967 - changing values in afterCreate/afterAttach when node is instantiated from view', () => {
  const Answer = types
    .model('Answer', {
      title: types.string,
      selected: false,
...
it('#993-1 - after attach should have a parent when accesing a reference directly', () => {
  const L4 = types
    .model('Todo', {
      id: types.identifier,
      finished: false,
...
it('#993-2 - references should have a parent event when the parent has not been accesed before', () => {
  const events: string[] = [];
  ...
  const expectedEvents = ['l1-ac', 'l2-ac', 'l2-at', 'l3-ac', 'l3-at', 'l4-ac', 'l4-at', 'onSnapshot', 'onSnapshot'];
...
it('should emit patches when applySnapshot is used', () => {
  const { Factory } = createTestFactories();
  const doc = Factory.create();
  const patches: IJsonPatch[] = [];
...
it('isAlive must be reactive', () => {
  const Todo = types.model('', { text: types.string });
  const todoListtore = types.model('', {
    todoList: types.array(Todo),
    todo: types.maybe(Todo),
  });

  const store = todoListtore.create({
    todoList: [{ text: '1' }, { text: '2' }],
    todo: { text: '3' },
  });

  unprotect(store);

  const t1 = store.todoList[0]!;
  const t2 = store.todoList[1]!;
  const t3 = store.todo!;
...
it('#1112 - identifier cache should be cleared for unaccessed wrapped objects', () => {
  configure({
    enforceActions: 'never',
  });
...
it('#1173 - detaching a model should not screw it', () => {
