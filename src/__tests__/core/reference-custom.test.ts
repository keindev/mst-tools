import { configure, reaction, values, when } from 'mobx';
...
it('should support custom references - basics', () => {
  configure({
...
it('should support custom references - adv', () => {
  const User = types.model('', {
    id: types.identifier,
    name: types.string,
  });
...
it('should support dynamic loading', done => {
  const events: string[] = [];
  const User = types.model('', {
    name: types.string,
    age: 0,
  ...
  const Store = types
    ...
    .actions(self => ({
      loadUser: flow(function* loadUser(name: string) {
        ...
        self.users.find(u => u.name === name)!.age = name.length * 3; // wonderful!
      }),
    }))
    .views(self => ({
...
it('custom reference / safe custom reference to another store works', () => {
  configure({
    enforceActions: 'never',
  });

  const Todo = types.model('', { id: types.identifier });
  const todoListtore = types.model('', { todoList: types.array(Todo) });
  const OtherStore = types.model('', {
    todoRef: types.maybe(
      types.reference(Todo, {
        get(id) {
          const node = resolveIdentifier(Todo, todoList, id);
  ...
  const todoList = todoListtore.create({
  ...
  otherStore.todoRef = todoList.todoList[2]!;
  otherStore.safeRef = todoList.todoList[2]!;
