...
it('should support prefixed paths in arrays', () => {
  configure({
    enforceActions: 'never',
  });
...
it('identifiers cannot be modified', () => {
  const Todo = types.model('', {
    id: types.identifier,
  });
...
it('should resolve refs during creation, when using path', () => {
  configure({
    enforceActions: 'never',
  });
...
it('should resolve refs over late types', () => {
  const Book = types.model('', {
    id: types.identifier,
    price: types.number,
  });
...
it('should resolve refs during creation, when using generic reference', () => {
  configure({
    enforceActions: 'never',
  });
...
it('identifiers should support subtypes of types.string and types.number', () => {
  const M = types.model('', {
...
it('string identifiers should not accept numbers', () => {
  const F = types.model('', {
    id: types.identifier,
  });
...
it('122 - identifiers should support numbers as well', () => {
  const F = types.model('', {
...
it('self reference with a late type', () => {
...
it('when applying a snapshot, reference should resolve correctly if value added after', () => {
...
it('should fail when reference snapshot is ambiguous', () => {
  configure({
    enforceActions: 'never',
  });
  ...
  expect(() => {
    ...
    "[mobx-state-tree] Cannot resolve a reference to type '(Box | Arrow)' with id: '2' unambiguously, there are multiple candidates: /boxes/1, /arrows/0"
  );
  unprotect(store);
  ...
  expect(err.message).toBe(
    // eslint-disable-next-line max-len
    "[mobx-state-tree] Cannot resolve a reference to type '(Box | Arrow)' with id: '1' unambiguously, there are multiple candidates: /boxes/0, /arrows/1"
  );
});

it('should support array of references', () => {
  const Box = types.model('', {
    id: types.identifierNumber,
    name: types.string,
  });
...
it('should restore array of references from snapshot', () => {
  const Box = types.model('', {
    id: types.identifierNumber,
    name: types.string,
  });
...
it('should support map of references', () => {
  const Box = types.model('', {
    id: types.identifierNumber,
    name: types.string,
  });
...
it('should restore map of references from snapshot', () => {
  const Box = types.model('', {
    id: types.identifierNumber,
    name: types.string,
  });
...
it('should support relative lookups', () => {
...
it('References are non-nullable by default', () => {
  const Todo = types.model('', {
    id: types.identifierNumber,
  });
  const Store = types.model('', {
...
it('References are described properly', () => {
  const Todo = types.model('', {
    id: types.identifierNumber,
  });
  const Store = types.model('', {
...
it('References in recursive structures', () => {
...
it('should applyPatch references in array', () => {
...
it('should applySnapshot references in array', () => {
  const Item = types.model('Item', {
    id: types.identifier,
    name: types.string,
  });
...
it('array of references should work fine', () => {
...
it('#1052 - Reference returns destroyed model after subtree replacing', () => {
  ...
  try {
    store.select(store.todoList.items[0]!);
...
it('tryReference / isValidReference', () => {
  ...
  unprotect(store);
  store.ref1 = store.todoList[0]!;
  store.ref2 = store.todoList[1]!;
  store.ref3 = store.todoList[2]!;
