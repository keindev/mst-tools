...
import {
    applySnapshot, clone, destroy, getSnapshot, IAnyStateTreeNode, Instance, OnReferenceInvalidated,
    OnReferenceInvalidatedEvent, ReferenceIdentifier, unprotect,
} from '../../mst/index';
...
const createStore = (
  ...
  const refOptions = {
    ...
    get(identifier: ReferenceIdentifier, parent: IAnyStateTreeNode | null) {
      return (parent as Instance<typeof Store>).todoList.find(t => t.id === identifier);
    },
...
for (const customRef of [false, true]) {
  describe(`onInvalidated - customRef: ${customRef}`, () => {
    it('from snapshot without accessing the referenced node', () => {
      configure({
    ...
    it('applying snapshot without accesing the referenced node', () => {
      configure({
        enforceActions: 'never',
      });
    ...
    it('runtime change', () => {
      ...
      expect(calls).toBe(0);
      store.onInv = store.todoList[1]!;
      expect(calls).toBe(0);
      store.onInv = store.todoList[0]!;
      expect(calls).toBe(0);
      store.todoList.remove(store.todoList[0]!);
      expect(calls).toBe(1);
      expect(ev!.parent).toBe(store);
      expect(oldRefId).toBe('1');
      expect(ev!.removeRef).toBeTruthy();
    ...
    it('replacing ref', () => {
      ...
      const onInv: OnReferenceInvalidated<Instance<typeof Todo>> = ev1 => {
        ...
        expect(ev1.invalidId).toBe(oldRefId);
        ev = ev1;
        ev1.replaceRef(store.todoList[1]);
      };
      const store = createStore({}, onInv);
    ...
    it('cloning works', () => {
      ...
      expect(calls).toBe(0);
      store1.onInv = store1.todoList[0]!;
      expect(calls).toBe(0);
      ...
      expect(store1.onInv).toBe(store1.todoList[0]);
    });
  });
}
...
describe('safeReference', () => {
  it('model property', () => {
    ...
    store.single = store.todoList[0]!;
    expect(store.single).toBe(store.todoList[0]);
    store.todoList.remove(store.todoList[0]!);
    expect(store.single).toBeUndefined();
  });

  it('deep model property', () => {
    configure({
      enforceActions: 'never',
    });
    ...
    store.deep.single = store.todoList[0]!;
    expect(store.deep.single).toBe(store.todoList[0]);
    store.todoList.remove(store.todoList[0]!);
    expect(store.deep.single).toBeUndefined();
  });
  ...
  it('array child', () => {
    ...
    store.arr.push(store.todoList[0]);
    store.arr.push(store.todoList[2]);
  ...
  it('map child', () => {
    ...
    store.map.set('a', store.todoList[0]);
    store.map.set('c', store.todoList[2]);
  ...
  it('invalid references in a snapshot should be removed', () => {
  ...
  it('setting it to an invalid id and then accessing it should still result in an error', () => {
    const store = createStore({});
    let s;
...
describe('safeReference with acceptsUndefined: false', () => {
  ...
  it('removes invalidates items from map/array', () => {
    ...
    const Store = types.model('', {
      todoList: types.array(MyRefModel),
...
it('#1275 - removing an object from a map should result in the snapshot of references being modified', () => {
  configure({
    enforceActions: 'never',
  });
