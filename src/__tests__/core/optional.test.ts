...
import { applySnapshot, cast, getSnapshot, unprotect } from '../../mst/index';

it('should provide a default value, if no snapshot is provided', () => {
...
it('should use the snapshot if provided', () => {
  const Row = types.model('', {
    name: '',
    quantity: 0,
  });
...
it('should throw if default value is invalid snapshot', () => {
  const Row = types.model('', {
    name: types.string,
...
it('should throw bouncing errors from its sub-type', () => {
  const Row = types.model('', {
    name: types.string,
    quantity: types.number,
  });
...
it('should accept a function to provide dynamic values', () => {
  let defaultValue = 1;
  const Factory = types.model('', {
...
it('Values should reset to default if omitted in snapshot', () => {
  configure({
...
it('optional frozen should fallback to default value if snapshot is undefined', () => {
...
it('an instance is not a valid default value, snapshot or function that creates instance must be used', () => {
...
it('undefined can work as a missing value', () => {
