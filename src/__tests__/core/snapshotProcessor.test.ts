...
import { cast, clone, detach, getNodeId, getSnapshot, SnapshotIn, unprotect } from '../../mst/index';
...
describe('snapshotProcessor', () => {
  ...
  describe('assigning instances works', () => {
    ...
    it('using instances in maps work', () => {
      const store = Store.create();
      const todo = TodoWithProcessor.create({ id: 'map' });

      store.addTodo(todo);

      expect(store.todoList.size).toBe(1);
      expect(getSnapshot(store.todoList)).toEqual({ map: { id: 'map' } });
    });

    it('using instances as values works', () => {
      const store = Store.create();
      const todo = TodoWithProcessor.create({ id: 'map' });
    ...
    it('using the non processed type in place of the processed one works', () => {
      const store = Store.create();
    ...
    it('using the processed type in place of the non processed one works', () => {
      const store = types
  ...
  it('cached initial snapshots are ok', () => {
  ...
  describe('1776 - reconciliation in an array', () => {
    it('model with transformed property is reconciled', () => {
    ...
    it('model with transformed identifier attribute is reconciled', () => {
      const SP = types.snapshotProcessor(
        types.model('', {
          id: types.identifier,
        }),
  ...
  describe('single node reconcilication', () => {
    ...
    it('model with transformed identifier property is reconciled', () => {
      const SP = types.snapshotProcessor(
        types.model('', {
          id: types.identifier,
        }),
    ...
    it('1791 - model wrapped with maybe is reconciled', () => {
      const SP = types.snapshotProcessor(
        types.model('', {
          id: types.identifier,
          x: types.number,
    ...
    it('model wrapped with optional is reconciled', () => {
      const SP = types.snapshotProcessor(
        types.model('', {
          id: types.identifier,
          x: types.number,
  ...
  it('1777 - preProcessor wrapped in maybe accepts undefined', () => {
    const SP = types.snapshotProcessor(
      types.model('', {
        id: types.identifier,
        x: types.number,
