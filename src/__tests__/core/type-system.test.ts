...
it('should recognize a valid snapshot', () => {
...
it('should recognize an invalid snapshot', () => {
  const { Box } = createTestFactories();

  expect(Box.is({ width: '1', height: '2' })).toEqual(false);
});

it('should check valid nodes as well', () => {
  const { Box } = createTestFactories();
...
it('should check invalid nodes as well', () => {
  const { Box } = createTestFactories();
  const doc = Box.create();

  expect(types.model('', { anotherAttr: types.number }).is(doc)).toEqual(false);
});
it('should do typescript type inference correctly', () => {
  configure({
  ...
  const A = types
    ...
    .actions(self => {
      ...
      function anotherMethod(_x: string) {}
...
it('#66 - it should accept superfluous fields', () => {
...
it('#66 - it should not require defaulted fields', () => {
  const Item = types.model('', {
    id: types.number,
...
it('#66 - it should be possible to omit defaulted fields', () => {
  const Item = types.model('', {
    id: types.number,
    name: 'boo',
  });
...
it('#66 - it should pick the correct type of defaulted fields', () => {
  const Item = types.model('', {
    id: types.number,
    name: 'boo',
  });
...
it('cannot create factories with null values', () => {
  expect(() =>
    types.model('', {
      x: null,
    } as any)
...
it('can create factories with maybe primitives', () => {
  const F = types.model('', {
...
it('is possible to refer to a type', () => {
  configure({
    enforceActions: 'never',
  });
...
it('.Type should not be callable', () => {
  const Todo = types
    .model('', {
      title: types.string,
    })
...
it('.SnapshotType should not be callable', () => {
  const Todo = types
    .model('', {
      title: types.string,
    })
...
it('types instances with compatible snapshots should not be interchangeable', () => {
  configure({
    enforceActions: 'never',
  });
  ...
  const Store = types
    ...
    .views(self => {
      ...
      return {
        ...
        getAmount(): number {
          return self.todoList.size + getActualAmount();
        },
      };
    })
    .actions(self => ({
      setAmount() {
        // const x: number = self.todoList.size + self.amount + self.getAmount();
        self.x = self.todoList.size + self.amount + self.getAmount();
      },
    }));
...
it('should provide detailed reasons why the value is not appicable', () => {
  const Todo = types
    .model('', {
      title: types.string,
    })
  ...
  const Store = types
    ...
    .views(self => ({
      ...
      getAmount(): number {
        return self.todoList.size + self.todoList.size;
      },
    }))
    .actions(self => ({
      setAmount() {
  ...
  expect(() =>
    Store.create({
      todoList: { '1': { title: true, setTitle: 'hello' } },
      amount: 1,
      getAmount: 'hello',
    } as any)
  ).toThrowError(
    // MWE: TODO: Ideally (like in MST =< 0.9):
    // at path "/todoList/1/setTitle" value \`"hello"\` is not assignable  (Action properties should not be provided in the snapshot).
    ...
    // eslint-disable-next-line max-len
    `[mobx-state-tree] Error while converting \`{"todoList":{"1":{"title":true,"setTitle":"hello"}},"amount":1,"getAmount":"hello"}\` to \`AnonymousModel\`:

    at path "/todoList/1/title" value \`true\` is not assignable to type: \`string\` (Value is not a string).`
  );
});

// eslint-disable-next-line jest/expect-expect
it('should type compose correctly', () => {
  const Car = types
    .model('', {
      wheels: 3,
    })
...
it('should extend {pre,post}ProcessSnapshot on compose', () => {
  ...
  const LoggableCar = _types.compose(CompositionTracker, Car, Logger).props({
    composedOf: types.array(types.string),
    composedWith: types.array(types.string),
  });
...
// eslint-disable-next-line jest/expect-expect
it('should extend types correctly', () => {
  const Car = types
    .model('', {
      wheels: 3,
    })
  ...
  // FIXME: remove TTT;
  type TTT = SnapshotIn<Instance<typeof LoggableCar>>;

  const LoggableCar = _types.compose('LoggableCar', Car, Logger);
  const x = LoggableCar.create({ wheels: 3, logNode: 'test' /* compile error: x: 7  */ } as TTT);

  // x.test() // compile error
  x.drive();
  x.log('z');
});
it('self referring views', () => {
...
it('#922', () => {
  expect(() => {
    const Stateable = types.model('Statable', {
      state: types.optional(types.enumeration('state', ['initalized', 'pending', 'done', 'error']), 'initalized'),
    });
    ...
    NonExtendedUser.create({ name: 'you' });
    User.create({ name: 'me' });
  }).not.toThrow();
});
...
it('#932', () => {
  ...
  const MyModel = types.model('MyModel', {
    myField: types.array(types.frozen<IMyInterface>()),
  });
...
// eslint-disable-next-line jest/expect-expect
it('snapshot type of reference must be string | number', () => {
...
it('castToSnapshot', () => {
  ...
  // eslint-disable-next-line @typescript-eslint/no-empty-function
  const secondModel = types.model('', { brew2: types.map(firstModel) }).actions(() => ({ do() {} }));
...
it('create correctly chooses if the snapshot is needed or not - #920', () => {
  const X = types.model('', {
    test: types.string,
  });
  const T = types.model('', {
...
it('#1117', () => {
  ...
  const Failsafe = <C, S, T>(
    t: IType<C, S, T>,
    handleProblem: (value: C, validationError: ReturnType<IType<C, S, T>['validate']>) => void = (value, error) => {
...
// eslint-disable-next-line jest/expect-expect
it('MST array type should be assignable to plain array type', () => {
  configure({
    enforceActions: 'never',
  });
...
// eslint-disable-next-line jest/expect-expect
it('can get snapshot from submodel (submodel is IStateNodeTree', () => {
  const T = types.model('', {
    a: types.model('', { x: 5 }),
  });
  const t = T.create({ a: {} });
  // TODO: remove any
  const sn = getSnapshot<any>(t.a).x;

  expect(sn).toBeDefined();
});

// eslint-disable-next-line jest/expect-expect
it('can extract type from complex objects', () => {
  const T = types.maybe(
    types.model('', {
      a: types.model('', {
        x: 5,
...
it('#1307 custom types failing', () => {
  ...
  expect(createCustomType).toBeDefined();
});
...
it('#1343', () => {
  ...
  function createTypeA<T extends ModelPropertiesDeclaration>(t: T) {
    return types.model('TypeA', t).views(() => ({
      get someView() {
        return null;
      },
    }));
  ...
  expect(createTypeA).toBeDefined();
  expect(createTypeB).toBeDefined();
});
...
// eslint-disable-next-line jest/expect-expect
it('#1307 optional can be omitted in .create', () => {
...
// eslint-disable-next-line jest/expect-expect
it('maybe / optional type inference verification', () => {
  const T = types.model('', {
    a: types.string,
    b: 'test',
