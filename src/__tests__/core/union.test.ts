...
import {
    applySnapshot, getSnapshot, getType, hasParent, Instance, setLivelinessChecking, SnapshotIn, tryResolve,
} from '../../mst/index';
...
it('should complain about multiple applicable types no dispatch method', () => {
...
it('should have parent whenever creating or applying from a complex data structure to a model which has Union typed children', () => {
  const { Block } = createTestFactories();
...
it('should complain about no applicable types', () => {
...
it('should be smart enough to discriminate by keys', () => {
  const { Box, Square } = createTestFactories();
...
it('should discriminate by value type', () => {
...
it('should compute exact union types', () => {
...
it('should compute exact union types - 2', () => {
  const { Box, DispatchPlane, Square } = createTestFactories();

  expect(DispatchPlane.is(Box.create({ width: 3, height: 2 }))).toEqual(true);
  expect(
    DispatchPlane.is(Square.create({ width: 3, height: 2 } as any /* incorrect type, superfluous attr!*/))
  ).toEqual(true);
});

it('should use dispatch to discriminate', () => {
  const { DispatchPlane } = createTestFactories();
  const a = DispatchPlane.create({ width: 3 });

  expect(getSnapshot(a)).toEqual({ width: 3 });
});

it('should eagerly match by ambiguos value', () => {
...
it('should eagerly match by ambiguos value - 2', () => {
...
it('should eagerly match by value literal', () => {
  const { ManWomanOrAll, All, Man } = createLiteralTestFactories();
...
it('dispatch', () => {
  ...
  const Num = types.union({ dispatcher: snapshot => (snapshot.value % 2 === 0 ? Even : Odd) }, Even, Odd);
...
// eslint-disable-next-line max-len, jest/expect-expect
it('961 - apply snapshot to union should not throw when union keeps models with different properties and snapshot is got by getSnapshot', () => {
...
describe('1045 - secondary union types with applySnapshot and ids', () => {
  ...
  function initTest(useSnapshot: boolean, useCreate: boolean, submodel1First: boolean, type: number) {
    ...
    const Submodel = submodel1First ? types.union(Submodel1, Submodel2) : types.union(Submodel2, Submodel1);
    ...
    return {
      ...
      applySn() {
        ...
        const expected = useSnapshot
          ? { id: sn.id, extraField1: sn.extraField1?.toUpperCase(), extraField2: sn.extraField2?.toUpperCase() }
          : sn;
            ...
            describe(useCreate ? 'using create' : 'not using create', () => {
              for (const type of [2, 1]) {
                describe(`snapshot is of type Submodel${type}`, () => {
                  ...
                  // eslint-disable-next-line jest/expect-expect, quotes
