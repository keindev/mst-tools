import { autorun, isObservable, isObservableProp, observable, reaction } from 'mobx';

import { types } from '../../index';
import { getSnapshot, recordPatches, unprotect } from '../../mst/index';
...
it('Properties should be readable and writable', () => {
  const i = Todo.create();
...
it('VS should not show up in snapshots', () => {
  expect(getSnapshot(Todo.create())).toEqual({ done: false });
});

it('VS should not show up in patches', () => {
  const i = Todo.create();
...
it('VS should not be deeply observable', () => {
  const i = types
    .model('', {})
    .volatile(() => ({
      x: { a: 1 },
...
it('VS should not be strongly typed observable', () => {
  const i = Todo.create();

  // TEST: type error i.state = 7
  // eslint-disable-next-line promise/catch-or-return, promise/prefer-await-to-then, promise/always-return, @typescript-eslint/no-empty-function
  i.state.then(() => {}); // it's a promise
  // TEST: not available on snapshot: getSnapshot(i).state
  expect(true).toBe(true);
});

it('VS should not be modifiable without action', () => {
  const i = Todo.create();
...
it('VS should expect a function as an argument', () => {
  expect(() => {
    types
      .model('', {})
      ...
      .volatile({ state: 1 })
      .create();
  }).toThrowError('You passed an object to volatile state as an argument, when function is expected');
});

it('VS should not be modifiable when unprotected', () => {
  const i = Todo.create();
...
it('VS sample from the docs should work (1)', () => {
...
it('VS sample from the docs should work (2)', () => {
  const T = types.model('', {}).extend(() => {
    let localState = 3;
